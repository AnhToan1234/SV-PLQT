...
def FindFileInDir(dirPath, pattern, fullPath=True, searchChildDir=False):
    ...
    for fullpathDir, subdirs, files in os.walk(dirPath):
        ...
        if (not searchChildDir and isChildDir):
            continue
        currentDirName = GetDirName(fullpathDir)
...
data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "training-model")

imgPaths = FindFileInDir(data_dir, "*.jpg", False, True)
...
model.save("model_CVs_local_9_8_750_500_36.h5")
